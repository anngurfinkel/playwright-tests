/* import { test, expect } from '@playwright/test';

test('1234567', async ({ page }) => {
  await page.goto('https://platform.labelyourdata.com/sign-up');
  await page.getByTestId('sign-up-email').click();
  await page.getByTestId('sign-up-email').fill('ahurfinkel@labelyourdata.com');
  await page.getByTestId('sign-up-username').click();
  await page.getByTestId('sign-up-username').fill('clientautotest_1');
  await page.getByTestId('sign-up-password').click();
  await page.getByTestId('sign-up-password').fill('Fjik67%ips');
  await page.getByTestId('sign-up-confirm_password').click();
  await page.getByTestId('sign-up-confirm_password').fill('Fjik67%ips');
  await page.getByRole('button', { name: 'Sign up', exact: true }).click();
});
*/